...
vtkStandardNewMacro(vtkPlaneExtend);

vtkCxxSetObjectMacro(vtkPlaneExtend,PlaneProperty,vtkProperty);

vtkPlaneExtend::vtkPlaneExtend() : vtkPolyDataSourceWidget()
{
  this->State = vtkPlaneExtend::Start;
  this->EventCallbackCommand->SetCallback(vtkPlaneExtend::ProcessEvents);
...
void vtkPlaneExtend::SetEnabled(int enabling)
{
  if ( ! this->Interactor )
    {
...
void vtkPlaneExtend::ProcessEvents(vtkObject* vtkNotUsed(object), 
                                   unsigned long event,
                                   void* clientdata, 
                                   void* vtkNotUsed(calldata))
{
  vtkPlaneExtend* self = reinterpret_cast<vtkPlaneExtend *>( clientdata );
...
void vtkPlaneExtend::PrintSelf(ostream& os, vtkIndent indent)
{
...
void vtkPlaneExtend::PositionHandles()
{
  double *o = this->PlaneSource->GetOrigin();
  double *pt1 = this->PlaneSource->GetPoint1();
  double *pt2 = this->PlaneSource->GetPoint2();
  ...
  this->HandleGeometry[0]->SetCenter(pt2);

  double x[3];
  x[0] = pt1[0] + pt2[0] - o[0];
  x[1] = pt1[1] + pt2[1] - o[1];
  x[2] = pt1[2] + pt2[2] - o[2];
  this->HandleGeometry[1]->SetCenter(x); //far corner
...
int vtkPlaneExtend::HighlightHandle(vtkProp *prop)
{
  ...
  if ( this->CurrentHandle )
    ...
    for (int i=0; i<42; i++) //find handle
      {
...
void vtkPlaneExtend::HighlightNormal(int highlight)
{
  if ( highlight )
    {
    this->ValidPick = 1;
...
void vtkPlaneExtend::HighlightPlane(int highlight)
{
  if ( highlight )
    {
    this->ValidPick = 1;
...
void vtkPlaneExtend::OnLeftButtonDown()
{
  ...
  if (!this->CurrentRenderer || !this->CurrentRenderer->IsInViewport(X, Y))
    {
    this->State = vtkPlaneExtend::Outside;
    return;
    }
  ...
  if ( path != NULL )
    {
    this->State = vtkPlaneExtend::Scaling;
    ...
    if ( path != NULL )
      ...
      if ( prop == this->ConeActor || prop == this->LineActor ||
        ...
        {
        this->State = vtkPlaneExtend::Rotating;
...
void vtkPlaneExtend::OnLeftButtonUp()
{
  if ( this->State == vtkPlaneExtend::Outside ||
       this->State == vtkPlaneExtend::Start )
    {
    return;
    }
...
void vtkPlaneExtend::OnMouseMove()
{
  ...
  // Process the motion
  if ( this->State == vtkPlaneExtend::Scaling )
    {
    // Okay to process
    if ( this->CurrentHandle )
      {
  ...
  else if ( this->State == vtkPlaneExtend::Rotating )
    {
...
void vtkPlaneExtend::MovePoint2(double *p1, double *p2)
{
  ...
  // The point opposite point (pt1) stays fixed
  double pt3[3];
  pt3[0] = pt1[0] + pt2[0] - o[0];
  pt3[1] = pt1[1] + pt2[1] - o[1];
  pt3[2] = pt1[2] + pt2[2] - o[2];
...
void vtkPlaneExtend::MovePoint3(double *p1, double *p2)
{
  //Get the plane definition
  double *o = this->PlaneSource->GetOrigin();
  double *pt1 = this->PlaneSource->GetPoint1();
...
void vtkPlaneExtend::Rotate(double *p1, double *p2, double *vpn)
{
  ...
  vtkMath::Cross(axis, v, rotAngle);
  double diagonal = sqrt(double(size[0]*size[0]+size[1]*size[1]));
  int sign = this->getThetaSign(axis, o, p1, p2);
  theta = sign * 360.0 * (vtkMath::Normalize(rotAngle) / diagonal);
...
void vtkPlaneExtend::CreateDefaultProperties()
{
  // Handle properties
...
void vtkPlaneExtend::PlaceWidget(double bds[6])
{
  int i;
...
void vtkPlaneExtend::SizeHandles()
{
...
void vtkPlaneExtend::SelectRepresentation()
{
  if ( ! this->CurrentRenderer )
    {
    return;
...
void vtkPlaneExtend::SetResolution(int r)
{
  this->PlaneSource->SetXResolution(r); 
  this->PlaneSource->SetYResolution(r); 
}

int vtkPlaneExtend::GetResolution()
{ 
...
void vtkPlaneExtend::SetOrigin(double x, double y, double z) 
{
  this->PlaneSource->SetOrigin(x,y,z);
  this->PositionHandles();
}

void vtkPlaneExtend::SetOrigin(double x[3]) 
{
  this->SetOrigin(x[0], x[1], x[2]);
}

double* vtkPlaneExtend::GetOrigin() 
{
  return this->PlaneSource->GetOrigin();
}

void vtkPlaneExtend::GetOrigin(double xyz[3]) 
{
...
void vtkPlaneExtend::SetPoint1(double x, double y, double z) 
{
  this->PlaneSource->SetPoint1(x,y,z);
  this->PositionHandles();
}

void vtkPlaneExtend::SetPoint1(double x[3]) 
{
  this->SetPoint1(x[0], x[1], x[2]);
}

double* vtkPlaneExtend::GetPoint1() 
{
  return this->PlaneSource->GetPoint1();
}

void vtkPlaneExtend::GetPoint1(double xyz[3]) 
{
...
void vtkPlaneExtend::SetPoint2(double x, double y, double z) 
{
  this->PlaneSource->SetPoint2(x,y,z);
  this->PositionHandles();
}

void vtkPlaneExtend::SetPoint2(double x[3]) 
{
  this->SetPoint2(x[0], x[1], x[2]);
}

double* vtkPlaneExtend::GetPoint2() 
{
  return this->PlaneSource->GetPoint2();
}

void vtkPlaneExtend::GetPoint2(double xyz[3]) 
{
  this->PlaneSource->GetPoint2(xyz);
}
// Description:
// Get the opposite corner of the origin.
double* vtkPlaneExtend::GetPoint3()
{
  double *o = this->PlaneSource->GetOrigin();
  double *pt1 = this->PlaneSource->GetPoint1();
  double *pt2 = this->PlaneSource->GetPoint2();
  ...
  vtkMath::Subtract(pt2, o, op2);
  vtkMath::Add(op1, op2, op3);
  vtkMath::Subtract(o, op3, pt3);
  return pt3;
}
...
void vtkPlaneExtend::SetCenter(double x, double y, double z) 
{
...
// Description:
// Set the center of the plane.
void vtkPlaneExtend::SetCenter(double c[3]) 
{
...
double* vtkPlaneExtend::GetCenter() 
{
  return this->PlaneSource->GetCenter();
}

void vtkPlaneExtend::GetCenter(double xyz[3]) 
{
...
void vtkPlaneExtend::SetNormal(double x, double y, double z) 
{
...
// Description:
// Set the normal to the plane.
void vtkPlaneExtend::SetNormal(double n[3]) 
{
...
double* vtkPlaneExtend::GetNormal() 
{
  return this->PlaneSource->GetNormal();
}

void vtkPlaneExtend::GetNormal(double xyz[3]) 
{
  this->PlaneSource->GetNormal(xyz);
}

void vtkPlaneExtend::GetPolyData(vtkPolyData *pd)
{ 
  pd->ShallowCopy(this->PlaneSource->GetOutput()); 
}

vtkPolyDataAlgorithm *vtkPlaneExtend::GetPolyDataAlgorithm()
{
  return this->PlaneSource;
}

void vtkPlaneExtend::GetPlane(vtkPlane *plane)
{
  if ( plane == NULL )
    {
    return;
...
void vtkPlaneExtend::UpdatePlacement(void)
{
  this->PlaneSource->Update();
  this->PositionHandles();
}

int vtkPlaneExtend::getThetaSign(double *axis, double *o, double *p1, double *p2)
{
  double *ov1 = new double[3];
  double *ov2 = new double[3];
  double *comparison = new double[3];
  vtkMath::Subtract(p1, o, ov1);
  vtkMath::Subtract(p2, o, ov2);
  vtkMath::Cross(ov1, ov2, comparison);
  double result = vtkMath::Dot(comparison, axis);
  if (result > 0) {
    return 1;
  } else {
    return -1;
